...
func PopulateClientPaths(prefix *gnmipb.Path, paths []*gnmipb.Path, path2URI *map[*gnmipb.Path]string, addWildcardKeys bool) error {
	opts := []translib.PathValidatorOpt{&translib.AppendModulePrefix{}}
	if addWildcardKeys {
		opts = append(opts, &translib.AddWildcardKeys{})
	}
...
func ConvertToURI(prefix *gnmipb.Path, path *gnmipb.Path, opts ...translib.PathValidatorOpt) (string, error) {
	fullPath := path
	if prefix != nil {
	...
	if err := translib.NewPathValidator(opts...).Validate(fullPath); err != nil {
		return "", err
	}
